...
export function ledgerCurve(data: EquityPoint[] | null | undefined, days: number, now = Date.now()): PerformancePoint[] {
  ...
  return sorted.map(([time, capital]) => ({ time, value: capital, returnPct: (capital / anchor - 1) * 100 }));
}
export function alignedComparison(equity?: CurvePoint[] | null, benchmark?: CurvePoint[] | null) {
  ...
  return { benchmark: other.at(-1)!.returnPct, difference: book.at(-1)!.returnPct - other.at(-1)!.returnPct };
}
