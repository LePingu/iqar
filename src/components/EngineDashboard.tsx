...
function Dashboard({ sessionId, mode }: { sessionId: string; mode: EngineMode }) {
  ...
  return <div className="live-dashboard">
    <header className="trading-header"><div className="trading-heading"><h1>{mode === 'real' ? 'Real trading' : 'Paper trading'}</h1><span className={`mode-label ${mode}`}>{mode === 'real' ? 'Real money' : 'Paper'}</span><span className="session-label"><i className={`engine-dot ${!statusQuery.error && status?.engine_alive ? 'is-alive' : ''}`} />{sessionId}</span></div>
