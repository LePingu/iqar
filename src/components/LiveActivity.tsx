...
export function AuditEvents({ sessionId }: { sessionId: string }) {
  ...
  const [active, setActive] = useState<'all' | 'active' | 'resolved'>('all');
  const filters = { category: category || undefined, severity: severity === 'all' ? undefined : severity as 'info' | 'warning' | 'critical', active: active === 'all' ? undefined : active === 'active' };
  const query = useInfiniteQuery({ queryKey: ['engineEvents', sessionId, filters], initialPageParam: undefined as string | undefined, queryFn: ({ pageParam }) => api.getEngineEvents(sessionId, { ...filters, cursor: pageParam }), getNextPageParam: (page, _pages, cursor) => page?.next_cursor && page.next_cursor !== cursor ? page.next_cursor : undefined, refetchInterval: 30_000 });
  ...
  return <div className="audit-events"><div className="activity-filter"><label>Severity <select className="input" value={severity} onChange={e => setSeverity(e.target.value)}><option value="all">All events</option><option value="critical">Critical</option><option value="warning">Warning</option><option value="info">Info</option></select></label><label>Category <select className="input" value={category} onChange={e => setCategory(e.target.value)}><option value="">All categories</option><option value="feed_stale">Feed stale</option><option value="exchange_unreachable">Exchange</option><option value="critic_degraded">Critic</option><option value="unpriceable_holdings">Unpriceable holdings</option><option value="lifecycle">Lifecycle</option><option value="control">Control</option><option value="order">Order</option><option value="reconciliation">Reconciliation</option></select></label><label>State <select className="input" value={active} onChange={e => setActive(e.target.value as typeof active)}><option value="all">All states</option><option value="active">Active</option><option value="resolved">Resolved</option></select></label><button className="btn btn-ghost" onClick={() => query.refetch()} disabled={query.isFetching}>Refresh</button></div>
    {query.isPending ? <p className="activity-empty">Loading audit events…</p> : query.error ? <p className="monitor-error" role="alert">{query.error.message}</p> : unavailable ? <p className="activity-empty">Structured audit events are not available from this backend yet.</p> : <>
      {!events.length && <p className="activity-empty">No matching events.</p>}
      {events.map(event => <button key={event.id} className={`audit-event ${context?.selection?.kind === 'event' && context.selection.event.id === event.id ? 'is-selected' : ''}`} onClick={() => context?.select({ kind: 'event', event })}><span className={`health-dot ${event.resolved_at ? 'healthy' : event.severity === 'critical' ? 'offline' : event.severity === 'warning' ? 'degraded' : 'unknown'}`} /><span><strong>{event.title ?? humanize(event.category)}</strong><small>{formatDate(event.occurred_at)} · {event.resolved_at ? 'Resolved' : 'Unresolved'}</small></span><span>↗</span></button>)}
