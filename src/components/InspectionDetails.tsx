...
import { formatDate, formatMoney, fmtPrice, formatPercentage, formatSignedMoney, formatTradePrice } from '../utils/trading';
import { humanize, measurement, rows, fillReturns } from '../utils/monitoring';
...
function OrderDetails({ source, orderId, showQuality = true }: { source: DecisionSource; orderId?: string | null; showQuality?: boolean }) {
  ...
  return <>
    {showQuality && <Quality value={order?.quality} />}
    ...
    {order && <details><summary>Order quantities & status</summary>{!showQuality && <><h4>Whole-order execution quality</h4><Quality value={order.quality} /></>}<dl className="telemetry-stats"><div><dt>Status</dt><dd>{humanize(order.status)}</dd></div><div><dt>Requested</dt><dd>{fmtPrice(order.requested_quantity)}</dd></div><div><dt>Filled</dt><dd>{fmtPrice(order.filled_quantity)}</dd></div><div><dt>Average price</dt><dd>{order.currency ? formatMoney(order.average_fill_price, order.currency) : fmtPrice(order.average_fill_price)}</dd></div><div><dt>Trigger</dt><dd>{humanize(order.trigger_type)}</dd></div></dl></details>}
  </>;
}
...
function FillDetails({ fill, currency, source, onSelect }: { fill: LiveFill; currency: string; source: DecisionSource; onSelect: (s: DecisionSelection) => void }) {
  ...
  return <>
    ...
    <dl className="fill-return-summary"><div><dt>Realized P&L</dt><dd className={returns.pnl == null ? '' : returns.pnl >= 0 ? 'text-positive' : 'text-negative'}>{formatSignedMoney(returns.pnl, currency)}</dd></div><div><dt>ROI</dt><dd className={returns.roi == null ? '' : returns.roi >= 0 ? 'text-positive' : 'text-negative'}>{formatPercentage(returns.roi)}</dd></div></dl>
    <Quality value={fill.execution_quality} />
    <OrderDetails source={source} orderId={fill.order_id} showQuality={false} />
    <details><summary>Fill details</summary><dl className="telemetry-stats"><div><dt>Quantity</dt><dd>{fmtPrice(fill.quantity)}</dd></div><div><dt>Price</dt><dd>{native ? `${fill.settle_currency} ${fmtPrice(fill.settle_price ?? fill.price)} (unconverted)` : formatTradePrice(fill.price, currency)}</dd></div><div><dt>Source</dt><dd>{fill.source ?? 'Not recorded'}</dd></div><div><dt>Commission (reported)</dt><dd>{fmtPrice(fill.commission)}</dd></div><div><dt>Reason</dt><dd>{humanize(fill.reason)}</dd></div><div><dt>Venue market</dt><dd>{fill.venue_market ?? 'Not recorded'}</dd></div>{fill.settle_currency && <div><dt>Native settlement</dt><dd>{fill.settle_currency} {fmtPrice(fill.settle_price)} · FX {fmtPrice(fill.settle_fx_rate)}</dd></div>}</dl></details>
