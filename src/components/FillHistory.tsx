...
import { FillsTable } from './LiveActivity';
import { rows } from '../utils/monitoring';
...
export function FillHistory({ sessionId, currency }: { sessionId: string; currency: string }) {
  ...
  const query = useInfiniteQuery({
    ...
    getNextPageParam: page => { const ids = rows(page?.fills).flatMap(f => f.id == null ? [] : [f.id]); return page?.fills?.length === 50 && ids.length ? Math.min(...ids) : undefined; },
    enabled: open && !invalid,
  });
  const seen = new Set<number>();
  const fills = query.data?.pages.flatMap(p => rows(p?.fills)).filter(fill => { if (fill.id == null) return true; if (seen.has(fill.id)) return false; seen.add(fill.id); return true; }) ?? [];
  return <section><button className="btn btn-ghost" aria-expanded={open} onClick={() => setOpen(!open)}>{open ? 'Hide fill history' : 'Browse fill history'}</button>{open && <>
    ...
    {invalid ? <p role="alert">Since must be before until.</p> : <>{query.isPending && <p>Loading history…</p>}{query.error && <p role="alert">{query.error.message}</p>}{query.data && <FillsTable fills={query.data.pages[0]?.fills == null ? null : fills} currency={currency} />}<button className="btn btn-ghost" disabled={!query.hasNextPage || query.isFetching} onClick={() => query.fetchNextPage()}>Load older fills</button></>}
  </>}</section>;
}
