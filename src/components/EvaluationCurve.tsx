...
import { alignedComparison, curvePoints, ledgerCurve, measured, rows, isStale } from '../utils/monitoring';
...
export function EvaluationCurve({ sessionId, mode, ledger, ledgerAsOf }: { sessionId: string; mode: EngineMode; ledger?: EquityPoint[] | null; ledgerAsOf?: string | null }) {
  ...
  const comparison = usingLedger ? null : alignedComparison(data?.equity, data?.btc);
  ...
  useEffect(() => {
    ...
    series.current = lines.map(([, title, color], i) => chart.addSeries(LineSeries, { title, color, lineWidth: 2, lineStyle: i === 3 ? 2 : 0, priceFormat: { type: 'custom', minMove: 0.01, formatter: (value: number) => `${value >= 0 ? '+' : ''}${value.toFixed(2)}%` } }));
  ...
  useEffect(() => {
    markerData.current = usingLedger ? [] : rows(data?.markers).filter(m => measured(m.book_index) && Number.isFinite(timestamp(m.timestamp))).sort((a, b) => timestamp(a.timestamp) - timestamp(b.timestamp));
    const values: SeriesMarker<Time>[] = markerData.current.map((m, i) => {
      ...
      return { id: String(i), time: timestamp(m.timestamp) as Time, price: m.book_index! - 100, position: 'atPriceMiddle', shape: m.side?.toLowerCase() === 'buy' ? 'arrowUp' : 'arrowDown', color: highlighted ? '#fff4c9' : m.side?.toLowerCase() === 'buy' ? '#58d59a' : '#f28b7d', size: highlighted ? 2 : 1 };
    });
    markers.current?.setMarkers(visible[0] ? values : []);
  }, [data, context?.highlighted, selectedDecision, visible, usingLedger]);
  const change = book.at(-1)?.value;
  ...
  return <GlassCard className="performance-panel">
    <div className="performance-heading"><div><h2>Portfolio performance</h2><div className="performance-return"><strong className={change == null ? '' : change >= 0 ? 'text-positive' : 'text-negative'}>{formatPercentage(change)}</strong>{comparison && <span>BTC {formatPercentage(comparison.benchmark)} · {comparison.difference >= 0 ? 'Ahead' : 'Behind'} by {Math.abs(comparison.difference).toFixed(2)} pp</span>}</div></div><div className="chart-periods" role="group" aria-label="Chart period">{[7, 30, 90].map(days => <button key={days} aria-pressed={windowDays === days} onClick={() => { setWindowDays(days); fitted.current = false; }}>{days}D</button>)}</div></div>
    ...
    {usingLedger && <p className="chart-message">Ledger value change only. Return adjustments and comparable benchmarks are unavailable.</p>}
